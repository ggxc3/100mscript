...
export function renderFrequencyPanel(
  ...
  host.innerHTML = `<div class="section-head"><h2>Frekvencie · LTE + 5G</h2><span class="frequency-badge">Samostatný režim</span></div>
    ...
    <p class="section-note"><code>Operator_sedi</code> kontroluje iba strednú frekvenciu f. <code>Operator_sedi_BW</code> kontroluje stred aj oba krajné body: f, f − BW a f + BW. Pri prázdnom alebo nulovom BW sú oba výsledky rovnaké. BW je odchýlka na každú stranu v MHz. Pri načítaných filtroch je výsledok <strong>yes</strong>, iba ak sa bod zhoduje s filtrom a filter nemení operátora. Bod mimo všetkých pásiem alebo s iným operátorom má <strong>no</strong>. Bez načítaných filtrov je výsledok <strong>yes</strong>. Filtre nemenia ani neduplikujú merania.</p>
