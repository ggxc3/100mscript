...
4. Pre každú kombináciu **zóna/úsek + MCC + MNC + frekvencia + technológia** sa vyberie jeden pôvodný riadok s **najvyšším jednotlivým RSRP**. Nejde o priemer. Pri rovnakom RSRP vyhrá prvý riadok po stabilnom časovom zoradení; pri zhodnom alebo chýbajúcom čase sa zachová vstupné poradie. LTE a 5G zostávajú samostatné aj pri rovnakej frekvencii.
5. Filtre sú **spoločné pre LTE aj 5G**, rovnako ako v štandardnom režime. Automaticky sa načítajú oba priečinky `filters/` aj `filtre_5G/`; ručne pridané súbory sa pridajú do jedného spoločného zoznamu. Rovnaký súbor vybraný viackrát sa načíta len raz. Rozhodujú podmienky filtra, nie jeho priečinok alebo technológia merania. Ak chceš použiť iba vlastné filtre, vypni automatické filtre a pridaj ich do spoločného zoznamu. BW a mapovanie stĺpcov zostávajú nezávislé pre LTE a 5G. Filtre **nič nemenia ani neduplikujú**. Kontroluje sa operátor už po oprave MNC na vybranom najsilnejšom meraní. Vyberá sa pravidlo s najviac zhodnými podmienkami, pri zhode podľa názvu súboru, rovnako ako v štandardnom režime.
6. Pri načítaných filtroch obsahuje `Operator_sedi` hodnotu `yes`, iba ak existuje zhodné pravidlo a nemení MCC ani MNC. Chýbajúca zhoda alebo zmena operátora (aj v niektorej z viacerých assignment kombinácií) znamená `no`. S vypnutými filtrami je výsledok `yes`.
7. **Dva výsledné stĺpce:** `Operator_sedi` kontroluje vždy iba strednú frekvenciu **f**. `Operator_sedi_BW` kontroluje **f, f − BW × 1 000 000 a f + BW × 1 000 000** a obsahuje `yes`, iba ak na všetkých troch bodoch existuje zhodné pravidlo a nemení operátora (pri vypnutých filtroch je výsledok `yes`). **BW** sa zadáva samostatne pre LTE a 5G v MHz, predvolene 0. Pri prázdnom alebo nulovom BW sú oba výsledky rovnaké. BW je odchýlka **na každú stranu**, nedelí sa dvoma a nepreberá sa zo vstupného stĺpca `BW`. Vnútro intervalu sa nekontroluje; frekvencia použitá na zoskupovanie sa nemení.

Rozsahy filtrov sú **[od, do)**: dolná hranica patrí do rozsahu, horná už nie. Rovnosť alebo rozsah s rovnakými hranicami kontroluje presnú hodnotu. Napríklad 5G meranie `MNC=2`, `2131,25 MHz`, `BW=5 MHz` má s dodanými filtrami `Operator_sedi=yes` a `Operator_sedi_BW=no`: dolný bod `2126,25 MHz` patrí do filtra Orange (`MNC=1`). Pri načítaných filtroch znamená bod mimo všetkých pásiem `no`. Ak nie sú načítané žiadne filtre, výsledok je `yes`. Napríklad LTE Orange 2112,5 MHz s poskytnutým pásmom od 2110 MHz má pri BW 2,5 MHz výsledok `yes / yes`, ale pri BW 2,6 MHz `yes / no`: dolný bod 2109,9 MHz už leží mimo všetkých poskytnutých pásiem.

### Pridávanie a spájanie viacerých súborov

Hlavičky sa načítajú z úvodnej vzorky každého CSV (najviac 1 MiB). Náhľad nespracúva ani nespája všetky merania. Pri ďalšom pridávaní sa nezmenené hlavičky použijú z pamäte; zmena veľkosti alebo času úpravy súboru vyvolá nové načítanie. Novšia požiadavka zruší predchádzajúcu a existujúce nastavenie frekvencie/PLMN zostane zachované.

Až **Spustiť spracovanie** načíta všetky riadky vrátane dodatočných stĺpcov, ktoré sa objavia neskôr než vo vzorke. Automatická oprava PLMN preto kontroluje celý súbor. Viaceré súbory sa zladia podľa názvov stĺpcov a zoradia podľa času rovnakým mechanizmom ako v pôvodnom režime: prednosť má Date + Time, náhradou je UTC. Bez použiteľného času sa zachová vstupné poradie. Pôvodný súbor a číslo jeho riadku zostanú vo výstupe. Pri zhodnom RSRP rozhoduje prvý riadok po tomto stabilnom zoradení.
...
| `Operator_sedi` | Kontrola iba strednej frekvencie |
| `Operator_sedi_BW` | Kontrola stredu a oboch krajných bodov podľa BW danej technológie; pri prázdnom/nulovom BW rovnaká ako stred |
...
## Mobile režim (synchronizácia 5G NR z NSA LTE)

Mobile režim slúži na doplnenie informácie o 5G NR pokrytí do 5G datasetu na základe časovej synchronizácie s NSA LTE meraním. Vstupný (hlavný) CSV je 5G meranie, NSA LTE CSV je súbor s rovnakou trasou meraný v rovnakom čase.
...
Pre daný 5G riadok sa v NSA LTE lookup tabuľke binárnym vyhľadávaním nájdu všetky NSA LTE riadky v časovom okne:
...
Hľadanie prebieha najprv podľa presnej zhody MCC+MNC. Ak 5G riadok nemá MCC/MNC, použije sa globálny NSA LTE lookup cez všetkých operátorov.
...
100mscript/
  ...
  internal/backend/    -- celá výpočtová logika
    ...
    mobile_sync_native.go -- synchronizácia 5G NR z NSA LTE
...
**Ľavý panel:**
- **Vstupné dáta** -- správa zoznamu CSV súborov (pridať, odstrániť, vyčistiť), automatické načítanie náhľadu po pridaní. Voľba mobile režimu s NSA LTE súbormi. Správa filtrov (automatické + manuálne).
